...
echo "Using srvctl to shutdown the database..." >> $BCKLOG 2>&1
srvctl stop database -d asprod

date >> $BCKLOG 2>&1
#Log into RMAN utility
echo "Logging into RMAN utility to startup and mount db..." >> $BCKLOG 2>&1
rman target / << eof >> $BCKLOG 
startup mount;
 run {
...
#Use srvctl to stop the datbase
echo "Using srvctl to shutdown and start the database..." >> $BCKLOG 2>&1
srvctl stop database -d asprod
srvctl start database -d asprod
