...
#echo "$?"
# Need to trap this error: psql: FATAL:  database "meldb" does not exist  and cleanup tmpdir
##### trying to debug here if possible
...
echo  -e "from ${SCHEMAMAIN}.${TBL}') to 's3://uconnect.mydomain.com/${database}/main/unload/${DTSTMP}/${TBL}/' credentials 'aws_access_key_id=${AWS_ACCESS_KEY_ID};aws_secret_access_key=${AWS_SECRET_ACCESS_KEY}' MANIFEST ADDQUOTES ESCAPE GZIP NULL AS 'ttT4rss2j8';" >> $UNLOAD
echo  -e " ) FROM 's3://uconnect.mydomain.com/${database}/main/unload/${DTSTMP}/${TBL}/manifest' credentials 'aws_access_key_id=${AWS_ACCESS_KEY_ID};aws_secret_access_key=${AWS_SECRET_ACCESS_KEY}' REGION '${REGION}' MANIFEST REMOVEQUOTES ESCAPE GZIP COMPUPDATE OFF NULL AS 'ttT4rss2j8';" >> $COPY
